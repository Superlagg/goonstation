...
/mob/living/silicon/ghostdrone
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if(burn_return[HAS_EFFECT] & ITEM_EFFECT_WELD)
			...
			if(user.a_intent == INTENT_HARM && burn_return[EFFECT_RESULT] & ITEM_EFFECT_FAILURE)
				user.visible_message("<span class='alert'><b>[user] beats [src] with [W]!</b></span>")
				damage_blunt(W.force)
			else if(user.a_intent == INTENT_HARM && burn_return[EFFECT_RESULT] & ITEM_EFFECT_SUCCESS)
				user.visible_message("<span class='alert'><b>[user] burns [src] with [W]!</b></span>")
				damage_heat(W.force)
			else if (get_fraction_of_percentage_and_whole(src.health,src.health_max) < 33)
				...
				src.health = max(1,min(src.health + 5,src.health_max))
