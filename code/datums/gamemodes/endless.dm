/datum/game_mode/endless
	...
	var/datum/endless_controller/events = null
...
/datum/endless/generation_marker
	var/x
	var/y
	var/z
	var/probability
...
/datum/game_mode/endless/announce()
	boutput(world, "<B>The current game mode is - endless!</B>")
	boutput(world, "<B>You have been assigned to construct the next generation space station.</B>")
	boutput(world, "<B>The map will hopefully.</B>")

/datum/game_mode/endless/pre_setup()
	in_setup = 1
	for(var/client/C)
	...
	C.limit = -1
	enabled_jobs += C
	boutput(world, "<B><font color='red'>Setting up map for endless. This will not take a very long time.</font></B>")
...
/proc/spawn_edge_asteroid(var/grade)
	...
	logTheThing("debug", null, null, "<B>Marquesas/endless:</B> Generating asteroid at [showCoords(ax, ay, 1)].")
	initial.x = ax
	initial.y = ay
	initial.z = 1
...
/datum/game_mode/endless/process()
...
/datum/game_mode/endless/declare_completion()
	boutput(world, "<b><span color='red'>The end is here.</span></b>")
