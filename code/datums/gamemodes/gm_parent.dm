...
/datum/game_mode/proc/declare_completion()
	...
	var/gameState = current_state

	for (var/datum/mind/traitor in traitors)
		if (gameState == GAME_STATE_INTERMISSION)
			if (in_centcom(traitor.current) || isdead(traitor.current) || isVRghost(traitor.current) || isghostcritter(traitor.current))
				antags.Add(traitor)
		else
			antags.Add(traitor)
	...
	var/whoReadsThis = world
	if (gameState == GAME_STATE_INTERMISSION)
		var/list/groupToReadThis = list()
		for (var/mob/escapedPlayer in mobs)
			if (escapedPlayer.mind && !istype(escapedPlayer, /mob/new_player) && (escapedPlayer.client && in_centcom(escapedPlayer) || isdead(escapedPlayer) || isVRghost(escapedPlayer) || isghostcritter(escapedPlayer)))
				groupToReadThis.Add(escapedPlayer.client)
		whoReadsThis = groupToReadThis
	for (var/datum/mind/traitor in antags)
		try
			...
			if (traitor.special_role == "mindslave")
				boutput(whoReadsThis, "<B>[traitor_name] was a mindslave!</B>")
				continue // Objectives are irrelevant for mindslaves and thralls.
			else if (traitor.special_role == "vampthrall")
				boutput(whoReadsThis, "<B>[traitor_name] was a vampire's thrall!</B>")
				continue // Ditto.
			else
				if (traitor.late_special_role)
					boutput(whoReadsThis, "<B>[traitor_name] was a late-joining [traitor.special_role]!</B>")
				else if (traitor.random_event_special_role)
					boutput(whoReadsThis, "<B>[traitor_name] was a random event [traitor.special_role]!</B>")
				else
					boutput(whoReadsThis, "<B>[traitor_name] was a [traitor.special_role]!</B>")
				...
				if (traitor.special_role == "changeling" && traitor.current)
					...
					boutput(whoReadsThis, "<B>Absorbed DNA:</b> [dna_absorbed]")
				...
				if (traitor.special_role == "vampire" && traitor.current)
					...
					boutput(whoReadsThis, "<B>Blood acquired:</b>  [blood_acquired][isnum(blood_acquired) ? " units" : ""]")
				...
				if (traitor.special_role == "werewolf")
					...
					for (var/datum/objective/specialist/werewolf/feed/O in traitor.objectives)
						if (O && istype(O, /datum/objective/specialist/werewolf/feed/))
							boutput(whoReadsThis, "<B>No. of victims:</b> [O.mobs_fed_on.len]")
				...
				if (traitor.special_role == "hunter")
					...
					for (var/datum/objective/specialist/hunter/trophy/T in traitor.objectives)
						if (traitor.current && T && istype(T, /datum/objective/specialist/hunter/trophy))
							...
							boutput(whoReadsThis, "<B>Combined trophy value:</b> [S]")

				if (traitor.special_role == "blob")
					var/victims = traitor.blob_absorb_victims.len
					boutput(whoReadsThis, "<b>\ [victims <= 0 ? "Not a single person was" : "[victims] lifeform[s_es(victims)] were"] absorbed by them  <span class='success'>Players in Green</span></b>")
					if (victims)
						...
						boutput(whoReadsThis, absorbed_announce)
				...
				if (traitor.special_role == "traitor")
					...
					boutput(whoReadsThis, "<b>They purchased [purchases <= 0 ? "nothing" : "[purchases] item[s_es(purchases)]"] with their [syndicate_currency]![purchases <= 0 ? " [pick("Wow", "Dang", "Gosh", "Good work", "Good job")]!" : null]</b>")
					if (purchases)
						...
						if (surplus)
							...
							for (var/i in traitor.traitor_crate_items)
								item_detail += "[bicon(i:item)] [i:name], "
							item_detail = copytext(item_detail, 1, -2)
						boutput(whoReadsThis, item_detail)

				if (traitor.special_role == "spy_thief")
					var/purchases = traitor.purchased_traitor_items.len
					var/stolen = traitor.spy_stolen_items.len
					boutput(whoReadsThis, "<b>They stole [stolen <= 0 ? "nothing" : "[stolen] items"]!</b>")
					if (purchases)
						...
						boutput(whoReadsThis, stolen_detail)
						boutput(whoReadsThis, rewarded_detail)
				...
				for (var/datum/objective/objective in traitor.objectives)
					...
					if (objective.check_completion())
						boutput(whoReadsThis, "<B>Objective #[count]</B>: [objective.explanation_text] <span class='success'><B>Success</B></span>")
						...
						boutput(whoReadsThis, "<B>Objective #[count]</B>: [objective.explanation_text] <span class='alert'>Failed</span>")
			...
			if (traitorwin)
				...
				boutput(whoReadsThis, "<B>The [traitor.special_role] was successful!<B>")
			else
				boutput(whoReadsThis, "<B>The [traitor.special_role] has failed!<B>")
			...
			if (traitor.former_antagonist_roles.len)
				for (var/string in traitor.former_antagonist_roles)
					if (string == "mindslave")
						...
						boutput(whoReadsThis, "<B>[traitor_name] was a [string]!</B>")
		catch(var/exception/e)
			logTheThing("debug", null, null, "kyle|former-antag-runtime: [e.file]:[e.line] - [e.name] - [e.desc]")

	if(gameState == GAME_STATE_INTERMISSION)
		del(antags)	// So we dont just add on to the last list of baddies endlessly
		boutput(world, "<h3>ANTAGS DELETED</h3>")
	return 1
